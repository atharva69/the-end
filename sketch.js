...
function setup(){
  ...
  monkey = createSprite(windowWidth/2-300,450,20,50);
   monkey.scale = 0.11;
     ground2=createSprite(windowWidth/2,490,windowWidth,20)
...
function spawnfood() {
  if (World.frameCount%150===0) {
    var banana = createSprite(windowWidth-300, 375, 25, 25);
    banana.addImage("banana",banana_Image);
    banana.velocityX = -3;
    banana.scale = 0.05;
    bananaGroup.add(banana);
    banana.lifetime = 1034;
  }
}
function spawnobstacle() {
  if (World.frameCount%80===0) {
    var stone = createSprite(windowWidth-300, 450, 10, 40);
